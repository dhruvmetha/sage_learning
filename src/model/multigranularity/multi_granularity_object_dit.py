...
class ObjectPatchEmbed(nn.Module):
    """Multi-granularity patch embedding for object selection."""
    ...
    def forward(self, robot_mask: torch.Tensor, goal_mask: torch.Tensor,
        ...
        Args:
            ...
            noisy_target: (B, 1, 64, 64) - The noisy object mask to denoise
            
        Returns:
            tokens: (B, 770, dim) - concatenated token sequence
        """
        ...
        # Concatenate all tokens: [robot, goal, movable, static, noisy_target]
        tokens = torch.cat([robot_tokens, goal_tokens, movable_tokens, static_tokens, noisy_target_tokens], dim=1)
        return tokens  # (B, 770, dim)


class ObjectPositionalEncoding(nn.Module):
    """Type-aware positional encoding for object selection model."""
    ...
    def __init__(self, dim: int, img_size: int = 64):
        ...
        # Type embeddings: robot=0, goal=1, movable=2, static=3, noisy_target=4
        ...
        self.global_pos_embedding = nn.Parameter(torch.randn(1, 2, dim) * 0.02)
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            tokens_with_pos: (B, 770, dim) - tokens with positional encoding added
        """
        B, N, D = tokens.shape
        ...
        type_ids[514:770] = 4 # noisy_target (256 tokens)
        
        # Add type embeddings
        type_emb = self.type_embedding(type_ids).unsqueeze(0).expand(B, -1, -1)  # (B, 770, dim)
        ...
        tokens[:, 514:770] = tokens[:, 514:770] + spatial_pos  # noisy_target
        
        return tokens


class MultiGranularityObjectDiT(nn.Module):
    """Multi-granularity Diffusion Transformer for object selection."""
    ...
    def __init__(
        ...
        self.patch_embed = ObjectPatchEmbed(img_size, dim)
        
        # Positional encoding
        self.pos_encoding = ObjectPositionalEncoding(dim, img_size)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Predict noise for object selection - following standard DiT approach.
        
        Args:
            x: (B, 5, 64, 64) - [robot, goal, movable, static, noisy_object_mask]
            t: (B,) - timestep
            
        Returns:
            noise_pred: (B, 1, 64, 64) - predicted noise for object mask
        """
        B = x.size(0)
        
        # Split input channels (fixed 5-channel input)
        robot_mask = x[:, 0:1]      # (B, 1, 64, 64)
        goal_mask = x[:, 1:2]       # (B, 1, 64, 64)  
        movable_mask = x[:, 2:3]    # (B, 1, 64, 64)
        static_mask = x[:, 3:4]     # (B, 1, 64, 64)
        noisy_target = x[:, 4:5]    # (B, 1, 64, 64) - The noisy target to denoise
        
        # Multi-granularity patch embedding (now includes noisy target)
        tokens = self.patch_embed(robot_mask, goal_mask, movable_mask, static_mask, noisy_target)  # (B, 770, dim)
        
        # Add positional encoding
        tokens = self.pos_encoding(tokens)  # (B, 770, dim)
        ...
        noisy_target_tokens = tokens[:, 514:770]  # (B, 256, dim) - last 256 tokens
