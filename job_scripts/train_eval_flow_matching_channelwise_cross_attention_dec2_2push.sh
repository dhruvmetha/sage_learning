#!/bin/bash
#SBATCH --job-name=flowmatch_chxattn_dec2_2push_eval
#SBATCH --output=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/flowmatch_chxattn_dec2_2push_eval_%j.out
#SBATCH --error=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/flowmatch_chxattn_dec2_2push_eval_%j.err
...
# Train (Channelwise Cross-Attention) on dec2 2-push -> Evaluate on 2-push filtered manifest.
# Usage: sbatch train_eval_flow_matching_channelwise_cross_attention_dec2_2push.sh
# Resume: sbatch train_eval_flow_matching_channelwise_cross_attention_dec2_2push.sh /path/to/checkpoint.ckpt
...
ARCH_NAME="channelwise_cross_attn"
...
srun --ntasks=${SLURM_NTASKS:-4} --kill-on-bad-exit=1 python src/train_generative.py \
  --config-name=train_flow_matching_channelwise_cross_attention \
  trainer.devices=4 \
