#!/bin/bash
#SBATCH --job-name=flowmatch_film_dec2_2push_meanstd_eval
#SBATCH --output=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/flowmatch_film_dec2_2push_meanstd_eval_%j.out
#SBATCH --error=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/flowmatch_film_dec2_2push_meanstd_eval_%j.err
...
# Train (FiLM/VectorDenoiser, mean_std) on dec2 2-push (split into single-step suffix samples)
# -> Evaluate on manifest_2push_test_minus_1push_test_filtered.txt using sequential ML collection.
# Usage: sbatch train_eval_flow_matching_film_dec2_2push_mean_std.sh
# Resume: sbatch train_eval_flow_matching_film_dec2_2push_mean_std.sh /path/to/checkpoint.ckpt
...
STATS_FILE="${H5_ROOT}/stats_mean_std.json"
...
ARCH_NAME="film"
NORM_MODE="mean_std_dec2_2push_srcsplit"
