#!/bin/bash
#SBATCH --job-name=vdiff_chxattn_cg_dec2_2push_eval
#SBATCH --output=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/vdiff_chxattn_cg_dec2_2push_eval_%j.out
#SBATCH --error=/common/users/tdn39/Robotics/Mujoco/sage_learning/job_scripts/slurm_logs/vdiff_chxattn_cg_dec2_2push_eval_%j.err
...
# Train (Vector Diffusion Channelwise Cross-Attention + Coord Grid) on dec2 2-push -> Evaluate on 2-push filtered manifest.
# Usage: sbatch train_eval_vector_diffusion_channelwise_cross_attention_coordgrid_dec2_2push.sh
# Resume: sbatch train_eval_vector_diffusion_channelwise_cross_attention_coordgrid_dec2_2push.sh /path/to/checkpoint.ckpt
...
ARCH_NAME="vdiff_channelwise_cross_attn_coordgrid"
...
srun --ntasks=${SLURM_NTASKS:-4} --kill-on-bad-exit=1 python src/train_generative.py \
  ...
  model.network._target_=src.model.networks.channelwise_cross_attention_vector_denoiser.ChannelwiseCrossAttentionVectorDenoiserBackbone \
  model.network.vector_dim=3 \
  model.network.image_channels=7 \
  model.network.hidden_dim=256 \
  model.network.num_layers=6 \
  +model.network.num_heads=8 \
  +model.network.token_grid=8 \
